...
run_glue_job_task_1 = PythonOperator(
    ...
    op_args=['jeju_public_free_wifi'],  # Glue Job 이름
...
check_s3_task = PythonOperator(
    ...
    op_kwargs={
        ...
        'prefix': 'data/raw/jeju_public_free_wifi/',
    },
    dag=dag,
)
...
run_glue_crawler_task_1 = PythonOperator(
    ...
    op_kwargs={'crawler_name': 'jeju_public_free_wifi'},  # 키워드 인자 전달
    dag=dag,
)
...
run_glue_job_task_2 = PythonOperator(
    ...
    python_callable=run_glue_job,
    op_args=['jeju_public_free_wifi_transform'],  # Glue Job 이름
    op_kwargs={'script_args': {'--source_bucket': 'ip-jeju-airflow',
                               '--target_bucket': 'ip-jeju-airflow'}}
)
...
run_glue_crawler_task_2 = PythonOperator(
    ...
    python_callable=run_glue_crawler,
    op_kwargs={'crawler_name': 'jeju_public_free_wifi_transform'},  # 키워드 인자 전달
    dag=dag,
)
...
s3_to_redshift = S3ToRedshiftOperator(
    ...
    schema='jeju_local_infra',  # 적재할 Redshift 스키마
    table='jeju_public_free_wifi',    # 적재할 Redshift 테이블 이름
    s3_bucket='ip-jeju-airflow',
    s3_key='data/transformed/jeju_public_free_wifi/',
...
DATA_SET_ID = "4e361e40-5cda-469f-ae68-608a72fa0761"  # QuickSight 데이터 세트 ID
import datetime

# 현재 날짜와 시간으로 고유한 ingestionId 생성
INGESTION_ID = f"jeju-public-free-wifi-refresh-ingestion-{datetime.datetime.now().strftime('%Y%m%d%H%M%S')}"
